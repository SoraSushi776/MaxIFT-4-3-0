    ...
    public static class LevelManager
    {
        ...
        public static void GameOverNormal(bool complete)
        {
            ...
            if (GameState == GameStatus.Died || GameState == GameStatus.Completed || GameState == GameStatus.Moving)
                LevelUI.Instance.NormalPage(percentage, Player.Instance.BlockCount, Player.Instance.CrownCount, IsNewRecord(percentage));
            
            SavePlayerPrefs(percentage);
        }
        
        public static bool IsNewRecord(float percentage)
        {
            string sceneName = SceneManager.GetActiveScene().name;
            if (PlayerPrefs.HasKey(sceneName + "Percentage"))
            {
                float record = PlayerPrefs.GetFloat(sceneName + "Percentage");
                if (percentage > record) return true; else return false;
            }
            else return true;
        }

        private static void SavePlayerPrefs(float percentage)
        {
            int blockCount = Player.Instance.BlockCount;
            int crownCount = Player.Instance.CrownCount;
            string sceneName = SceneManager.GetActiveScene().name;
            
            PlayerPrefs.SetFloat(sceneName + "Percentage", percentage);
            PlayerPrefs.SetInt(sceneName + "BlockCount", blockCount);
            PlayerPrefs.SetInt(sceneName + "CrownCount", crownCount);
            PlayerPrefs.Save();
        }
