...
public class ScreenCaptureEditor : EditorWindow
{
    ...
    [MenuItem("Tools/DLFM/Screenshots/Open Window")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow(typeof(ScreenCaptureEditor));
    }

    [MenuItem("Tools/DLFM/Screenshots/Reveal in Explorer")]
    ...
    [MenuItem("Tools/DLFM/Screenshots/Take a Screenshot")]
    private static void TakeScreenshot()
    {
        Directory.CreateDirectory(directory);
        var sceneName = UnityEditor.SceneManagement.EditorSceneManager.GetActiveScene().name;
        var currentTime = System.DateTime.Now;
        var filename = sceneName + "_" + currentTime.ToString().Replace('/', '-').Replace(':', '_') + ".png";
